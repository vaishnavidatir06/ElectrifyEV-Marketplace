...
import { FiEdit, FiHome, FiMail, FiPhone, FiUser } from 'react-icons/fi';
...
export default function Profile() {
    ...
    const fetchWishlist = async () => {
        try {
            ...
            const response = await fetch(`http://51.79.225.217:5000/wishlist/${session.user.name}`);
            if (!response.ok) {
    ...
    useEffect(() => {
        // Fetch wishlist items when the component mounts or when session changes
        if (session) {
            fetchWishlist();
        }
    }, [session]);
